...
Turn a vague improvement goal into a ready-to-run `/loops:optimize` invocation. The hard part of loop engineering is not running the loop — it is choosing a metric that can't be gamed, a scope that can't wander, and a verifier that makes "done" mean something. This skill does that design work.
